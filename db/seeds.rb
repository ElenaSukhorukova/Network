...
Account.all.collect do |account|
  ...
  unless account.sender_invites.try(:any?) || account.receiver_invites.try(:any?)
    quantity.times do
      receiver_invite_id = Account.ids.delete_if{|id| id == sender_invite_id }.sample 

      unless Invite.find_by(sender_invite_id: sender_invite_id, receiver_invite_id: receiver_invite_id) ||
...
Account.all.collect do |account|
  unless account.f_partner_friendships.try(:any?) || account.s_partner_friendships.try(:any?)
    quantity = rand(5...15)
    invite_id = Invite.where(sender_invite: account, confirmed: 'not').ids.sample 
    if invite_id && invite = Invite.find(invite_id)
      quantity.times do
        receiver = invite.receiver_invite
        unless Friendship.find_by(f_partner_friendship: account, s_partner_friendship: receiver) ||
              Friendship.find_by(f_partner_friendship: receiver, s_partner_friendship: account)

          if Friendship.create f_partner_friendship: account, s_partner_friendship: receiver, invite: invite
...
Account.all.collect do |sender|
  unless sender.sender_messages.try(:any?)
    rand(20...40).times do
      ...
      if (receiver.visibility == 'everybody' || friendship) && Conversation.account_conversations(sender).count < 20
...
Account.all.collect do |account|
  unless account.groups.try(:any?)
    rand(1..10).times do
      ...
      Group.create name_group: name_group, description: description, visibility: visibility, group_creator_id: group_creator_id
    end
  end
end

Group.all.collect do |group|
  if group.hobbies.count < 3
